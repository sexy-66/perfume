...
- 文档状态：已确认；1.2 结构优化和首轮简单性能优化已完成；M6已冻结、需求暂缓
- 版本：0.12
- 日期：2026-08-09
...
M6冻结期间以数据库和最近删除为引用来源。M6恢复后若仍采用7份快照方案，再把有效备份清单加入引用扫描。数据规模只有几十条，清理时直接扫描引用即可，不维护容易失真的手工引用计数。
...
- 应用启动前的数据库初始化与媒体目录初始化并行执行；首帧只构建当前首页，首帧完成后再预热另外两个根页面，并在首帧后异步初始化同步运行时。后续开发不得把隐藏根页面、同步网络启动或其他非首屏工作重新放回首帧前串行执行；
- 底部导航保留圆角、边框、阴影和近不透明白色外观，但不使用`BackdropFilter`或大半径实时背景模糊，避免高刷新率设备滚动时持续增加GPU合成压力；
- 香方和推荐香方网格卡片按目标宽度解码图片，当前使用`cacheWidth: 720`；详情、预览和导出等确实需要清晰原图的页面不受此限制；
...
- 2026-08-09用户在小米15 Pro上报告推荐界面拖影和启动偏慢。本轮设备未通过ADB连接，因此代码优化已在Android 16模拟器验证，但小米15 Pro仍需安装Release模式APK做滚动和启动复核；
- 当前`release`构建仍使用调试签名，应用ID仍为`com.example.xiangfangbu`占位值，只用于Release模式性能和功能冒烟，不是M7正式签名交付包。
...
- 当前全量92项Flutter测试、`flutter analyze`、Profile APK和Release APK构建均通过；
...
技术方案已确认，M1至M5、1.2结构优化和首轮简单性能优化均已开发并验证完成。当前仍停留在M5完成后的维护状态；M6已冻结、需求暂缓，M7未启动。执行性能维护不表示M7已启动。
...
- M3已完成：编辑阶段自动保存和恢复、纯整数计算、香方草稿和正式版本、新建/重复/旧版本/顾客上次比例调配、系统补全、输入后即时推荐区间提醒、顾客历史、调配修改历史和记录复制已实现；
...
- 当前格式检查、静态分析、Flutter全量92项测试、Profile APK和Release APK构建均已通过；M5真机验收由用户确认通过，具体网络组合未单独记录；
...
- 当前Release APK只用于非Debug性能和功能冒烟，仍采用调试签名及占位应用ID，正式签名、版本和升级交付仍属于未启动的M7。
...
- 连续快速点击任何弹层入口只产生一个弹层；
- 当前`dart format --output=none --set-exit-if-changed .`、`flutter analyze`、92项`flutter test`、Profile APK和Release APK构建通过。
