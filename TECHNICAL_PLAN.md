...
- 文档状态：已确认；1.2 结构优化开发中；M6已冻结、需求暂缓
- 版本：0.11
- 日期：2026-08-08
...
- 后续回归应记录设备型号、网络组合、关键结果和错误日志，便于问题复现。
...
技术方案已确认，M1至M5已开发并验收完成。当前停留在M5完成状态；M6已冻结、需求暂缓，M7未启动。
...
- M2原版已完成；当前按1.2重构为单层香料，删除 SKU 和 SKU 推荐覆盖，香料直接保存图片，推荐配置与香方直接选择香料；
...
- M5模拟验收、格式检查、静态分析、Flutter全量75项测试和Debug APK构建均已通过；真机验收由用户确认通过，具体网络组合未单独记录；
...
- 连续快速点击任何弹层入口只产生一个弹层；
- `dart format --output=none --set-exit-if-changed .`、`flutter analyze`、`flutter test`和`flutter build apk --debug`通过。
